    ...
    return (
        <div className="bg-ivory min-h-screen" data-testid="worker-dashboard">
            <header className="border-b border-line bg-white/80 backdrop-blur-md sticky top-0 z-30">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-3 sm:py-0 sm:h-16 flex items-center justify-between flex-wrap gap-3">
                    <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-noir text-ivory flex items-center justify-center font-serif flex-shrink-0">{user.name?.[0] || "I"}</div>
                        <div>
                            <div className="font-serif text-base sm:text-lg text-noir leading-none">Salom, {user.name} {user.surname || ""}</div>
                            <div className="text-xs text-stone">Sotuvchi paneli</div>
                        </div>
                    </div>
                    <div className="flex gap-2 flex-wrap items-center">
                        <div className="hidden sm:flex text-xs text-stone bg-cream rounded-full px-3 py-1.5 items-center gap-1">
                            <ScanLine className="w-3.5 h-3.5 text-rose"/> Skaner doim faol
                        </div>
                        <Button onClick={() => { setScannedProduct(null); setOpenScanner(true); }} variant="outline" className="border-rose text-rose hover:bg-rose hover:text-ivory rounded-full text-xs sm:text-sm" data-testid="open-scanner">
                            <Camera className="w-4 h-4 sm:mr-1"/> <span className="hidden sm:inline">Kamera</span>
                        </Button>
                        <Button onClick={() => punch("checkin")} className="bg-noir text-ivory hover:bg-rose rounded-full text-xs sm:text-sm" data-testid="checkin-btn">
                            <LogIn className="w-4 h-4 sm:mr-1" /> <span className="hidden sm:inline">Ishga keldim</span>
                        </Button>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-6 sm:py-8 space-y-6 sm:space-y-8">
                <Tabs defaultValue="register">
                    ...
                    <TabsContent value="register" className="mt-6">
                        <div className="grid lg:grid-cols-5 gap-4 sm:gap-6">
                            {/* Mahsulotlar tanlash */}
                            <div className="lg:col-span-3 bg-white rounded-2xl border border-line p-4 sm:p-5">
                                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                                    <div className="font-serif text-base sm:text-lg text-noir">Mahsulot tanlash</div>
                                    <div className="hidden sm:block text-xs text-stone">Barkodni skanerda ursangiz — to'g'ridan-to'g'ri savatga qo'shiladi</div>
                                </div>
                                    ...
                                    {visibleProducts.map(p => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className={`w-full flex items-center gap-2 sm:gap-3 p-2 rounded-xl border transition-all text-left ${Number(p.stock ?? 0) <= 0 ? 'border-line bg-gray-50 opacity-50 cursor-not-allowed' : 'border-line hover:border-rose hover:bg-rose/5'}`}
                                                data-testid={`pick-product-${p.id}`}
                                            >
                                                <img src={p.image_url} alt={p.name} className="w-10 h-10 sm:w-12 sm:h-12 object-cover rounded-lg bg-cream flex-shrink-0"/>
                                                <div className="flex-1 min-w-0">
                                                    <div className="font-serif text-xs sm:text-sm text-noir line-clamp-1">{p.name}</div>
                                                    <div className="text-[10px] sm:text-xs text-stone">
                                                ...
                                                {Number(p.stock ?? 0) <= 0 ? <span className="text-xs text-rose font-bold">0</span> : <Plus className="w-4 h-4 text-rose flex-shrink-0"/>}
                                            </button>
                                        );
                                    })}
                            ...
                            <div className="lg:col-span-2 space-y-4">
                                <form onSubmit={submitSale} className="bg-white rounded-2xl border border-line p-4 sm:p-5 space-y-3" data-testid="sale-form">
                                    <div className="flex items-center gap-2 mb-1">
                                        <ShoppingCart className="w-4 h-4 sm:w-5 sm:h-5 text-rose"/>
                                        <div className="font-serif text-base sm:text-lg text-noir">Savat ({cart.length})</div>
                                    </div>
                                    {cart.length === 0 ? (
                                        <div className="text-stone text-xs sm:text-sm text-center py-6 sm:py-8 border-2 border-dashed border-line rounded-xl">
                                            Savat bo'sh — mahsulotni bosing yoki barkodni skanerda o'qing
                                        </div>
                                    ) : (
                                        <div className="space-y-2 max-h-[280px] sm:max-h-[320px] overflow-y-auto pr-1" data-testid="cart-items">
                                            {cart.map((c, idx) => {
                                                ...
                                                return (
                                                    <div key={idx} className="bg-white border border-line rounded-xl overflow-hidden" data-testid={`cart-item-${idx}`}>
                                                        ...
                                                        <div className="flex items-center gap-2 p-2 sm:p-2.5">
                                                            <img src={c.product.image_url} alt="" className="w-9 h-9 sm:w-10 sm:h-10 object-cover rounded-lg bg-cream flex-shrink-0"/>
                                                            <div className="flex-1 min-w-0">
                                                                <div className="font-medium text-xs sm:text-sm text-noir line-clamp-1">{c.product.name}</div>
                                                                <div className="flex items-center gap-1.5 mt-0.5">
                                                                    {totalDisc > 0 && (
                                                                        <span className="text-[9px] sm:text-[10px] text-stone line-through">{originalUnit.toLocaleString()}</span>
                                                                    )}
                                                                    <span className="text-[10px] sm:text-xs font-semibold text-noir">{unit.toLocaleString()} so'm</span>
                                                                    {totalDisc > 0 && (
                                                                        <span className="text-[9px] sm:text-[10px] bg-rose/10 text-rose rounded px-1 py-0.5">−{totalDisc}%</span>
                                                                    )}
                                                                </div>
                                                            </div>
                                                        ...
                                                        {/* Miqdor + Chegirma */}
                                                        <div className="border-t border-line/60 bg-cream/50 px-2 sm:px-2.5 py-2 flex items-center gap-1.5 sm:gap-2 flex-wrap sm:flex-nowrap">
                                                            {/* Miqdor */}
                                                            <div className="flex items-center rounded-lg border border-line bg-white overflow-hidden flex-shrink-0">
                                                                <button type="button" onClick={() => updateCartItem(idx, { quantity: Math.max(1, c.quantity - 1) })} className="w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center hover:bg-rose hover:text-ivory transition-colors text-stone">
                                                                ...
                                                                <input
                                                                    ...
                                                                    className="w-7 sm:w-9 text-center text-xs sm:text-sm font-semibold border-0 bg-transparent focus:outline-none text-noir"
                                                                    data-testid={`cart-qty-${idx}`}
                                                                />
                                                                <button type="button" onClick={() => updateCartItem(idx, { quantity: c.quantity + 1 })} className="w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center hover:bg-rose hover:text-ivory transition-colors text-stone">
                                                                    <Plus className="w-2.5 h-2.5 sm:w-3 sm:h-3"/>
                                                                </button>
                                                            </div>

                                                            <div className="text-xs text-stone hidden sm:block">×</div>
                                                            ...
                                                            <div className="flex items-center gap-1 flex-1 flex-wrap">
                                                                {[0, 1, 2, 3, 5, 10].map(pct => (
                                                                    <button
                                                                        ...
                                                                        className={`h-5 sm:h-6 px-1.5 sm:px-2 rounded text-[10px] sm:text-[11px] font-medium transition-all border ${
                                                                ...
                                                                <div className="flex items-center gap-1">
                                                                    <input
                                                                        ...
                                                                        className="h-5 sm:h-6 w-16 sm:w-20 px-1.5 sm:px-2 text-[10px] sm:text-[11px] rounded border border-line bg-white focus:outline-none focus:border-rose text-noir placeholder:text-stone/50"
                                                                    />
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        {/* Jami */}
                                                        <div className="px-2 sm:px-2.5 py-1.5 flex justify-end">
